    ...
    try {
      ...
      if (result.success) {
        const formData = {
          ...
          instructor: result.data.instructor ? (result.data.instructor.username || result.data.instructor.name) : "",
          price: result.data.price,
          description: result.data.description,
          y_link: result.data.y_link, // Note: Backend DTO might not return this if not added to Entity yet
          p_link: result.data.thumbnail || result.data.p_link, // Handle thumbnail/p_link alias
        };
    ...
    try {
      ...
      if (isEditMode) {
        const editData = {
          ...
          instructorId: authService.getCurrentUser().id, // Assign to current user or keep existing? For edit, maybe different.
          // For now, let's assume edit doesn't change instructor or handle it separately.
          // Spec requires Create fix mainly.
          price: values.price,
          description: values.description,
          thumbnail: values.p_link,
          videoLink: values.y_link,
        };
