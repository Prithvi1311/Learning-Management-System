...
function Courses() {
        ...
        if (userId) {
          ...
          if (enrollmentsRes.success) {
            ...
            enrollmentsRes.data.forEach(item => {
              enrolledIds.push(item.course_id);
              if (item.completed) completedIds.push(item.course_id);
            });
  ...
  const filteredAndSortedCourses = useMemo(() => {
    let filtered = courses.filter(course => {
      ...
      const instructorName = (typeof course.instructor === 'object' && course.instructor) ? (course.instructor.username || "") : (course.instructor || "");
    ...
    filtered.sort((a, b) => {
      const getInst = (inst) => (typeof inst === 'object' && inst) ? (inst.username || "") : (inst || "");
      const getName = (name) => name || "";

      switch (sortBy) {
        case "name":
          return getName(a.course_name).localeCompare(getName(b.course_name));
        case "instructor":
          return getInst(a.instructor).localeCompare(getInst(b.instructor));
        case "price":
              ...
              {displayedCourses.map((course) => (
                ...
                >
                  <div className="relative overflow-hidden">
                    <img
                      src={course.thumbnail || course.p_link}
                      alt={course.course_name}
                      className="h-48 w-full object-cover group-hover:scale-105 transition-transform duration-200"
                    />
                    <span className="bg-white/90 backdrop-blur-sm text-gray-700 px-2 py-1 rounded-full text-xs font-medium">
                      ₹{(course.price * 10).toLocaleString()}
                    </span>
                  </div>

                  <div className="p-6">
                    <h3 className="text-xl font-bold text-gray-900 mb-3 line-clamp-2 group-hover:text-blue-600 transition-colors duration-200">
                      {(course.course_name || "Untitled").length < 8 ? `${course.course_name || "Untitled"} Tutorial` : (course.course_name || "Untitled")}
                    </h3>

                    <p className="text-gray-500 text-sm mb-6 flex items-center">
                      <span className="w-2 h-2 bg-blue-500 rounded-full mr-2"></span>
                      by {typeof course.instructor === 'object' && course.instructor ? (course.instructor.username || "Unknown") : (course.instructor || "Unknown")}
                    </p>

                    {completed.includes(course.course_id) ? (
                      <button
                        onClick={() => navigate("/learnings")}
                        className="w-full py-3 rounded-xl bg-green-100 text-green-700 font-semibold hover:bg-green-200 transition-all duration-200 border border-green-200 shadow-md"
                      >
                        ✓ Completed
                      </button>
                      ...
                      <button
                        onClick={() => navigate("/learnings")}
                        className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 font-semibold hover:from-blue-100 hover:to-indigo-100 transition-all duration-200 border border-blue-200 shadow-md"
                      >
                        ✓ Enrolled
                      </button>
                    ) : (
