...
const Performance = () => {
  ...
  const handleCertificateDownload = async (courseId, courseName) => {
    setDownloadingCert(courseId);
    const userId = localStorage.getItem("id");

    try {
      const response = await certificateService.downloadCertificate(userId, courseId);
      if (response.success) {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          {data.marks > 0 ? (
                            <button
                              onClick={() => handleCertificateDownload(data.course.course_id, data.course.course_name)}
